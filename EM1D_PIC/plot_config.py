...
waves_of_interest = 0  # (0 for electron waves, 1 for ion waves)
# lines to be overlaid on the plots (use None for no line, "auto" only works for theta = 0 or 90)
overlays = (("auto", 148, 148), ("auto", 148, 148))
orders_of_magnitude = 2  # orders of magnitude in the color bar
maximum_magnitude = 2  # use to lower the maximum order of magnitude in the color bar
plot_range_x = (0, 0.1)  # x-axis plot range (input: "full" for full plot)
plot_range_y = (0, 0.25E8) # y-axis plot range (input: "full" for full plot)
...
plot_species = (0,)  # species to plot in phase space
plot_colors = ("black",)  # color to use for each specie, same order as plot_species
specie_of_interest = 0  # index of the specie to focus on in the phase space animation (set to None to not focus)
